...
export const permissions = {
  ...
  // Local-prototype policy. This is intentionally centralized so it can become
  // an admin-only rule when authenticated authorization is introduced.
  ...
  canResolveRequest: (user:User, project:Project, _request:HelpRequest) => user.access==='admin'||project.teamIds.includes(user.id),
};
