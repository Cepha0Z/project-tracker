...
export const projectService = {
  ...
  create(data:AppData, actorId:string, input:{name:string;location?:string;principalId:string;leadId:string;teamIds?:string[];description?:string;deadline?:string}):AppData {
    ...
    if(![input.principalId,input.leadId,...selected].every(id=>data.users.some(u=>u.id===id&&isConnectedPerson(u))))return data;
  ...
  addTeamMember(data:AppData, actorId:string, projectId:string, userId:string):AppData {
    ...
    const member=data.users.find(u=>u.id===userId); if(!member||!isConnectedPerson(member))return data; const next={...data,projects:data.projects.map(p=>p.id===projectId?{...p,teamIds:[...p.teamIds,userId]}:p)};
