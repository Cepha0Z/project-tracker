...
export const workItemService = {
  ...
  update(data:AppData, actorId:string, itemId:string, changes:Partial<WorkItem>):AppData {
    ...
    if(!item||!project||!actor||!(permissions.canManageWorkItem(actor,project)||permissions.canUpdateOwnWork(actor,item))) return data;
  ...
  setStatus(data:AppData, actorId:string, itemId:string, status:WorkStatus, progress?:number, reason?:string):AppData { const item=data.workItems.find(w=>w.id===itemId)!,project=data.projects.find(p=>p.id===item.projectId)!,actor=data.users.find(u=>u.id===actorId); if(!actor||!(permissions.canManageWorkItem(actor,project)||permissions.canUpdateOwnWork(actor,item))) return data; const stamp=new Date().toISOString(),nextProgress=status==='Completed'?100:status==='In Progress'&&item.status==='Completed'?(progress??item.previousProgress??75):(progress??item.progress),timing=transition(item,status,nextProgress,stamp); const next={...data,workItems:data.workItems.map(w=>w.id===itemId?{...w,...timing,previousProgress:status==='Completed'?item.progress:w.previousProgress,blockedReason:status==='Blocked'?reason:undefined}:w)}; const verb=status==='Completed'?'completed':item.status==='Completed'?'reopened':status==='Blocked'?'blocked':item.status==='Not Started'&&status==='In Progress'?'started':'changed the status of'; return withActivity(next,item.projectId,actorId,`${verb} ${item.name}`); },
  setProgress(data:AppData,actorId:string,itemId:string,progress:number):AppData {
    ...
    const status:WorkStatus=value===100?'Completed':item.status==='Blocked'?'Blocked':value>0||item.status==='Completed'?'In Progress':'Not Started';
