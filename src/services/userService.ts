...
export const userService={
  create(data:AppData,actorId:string,input:{name:string;title:string;reportsToId?:string;access?:User['access']}):AppData{
    ...
    const user:User={id:makeId('person'),name:input.name.trim(),initials,title:input.title.trim(),access:input.access||'employee',reportsToId:input.reportsToId||undefined,loginEnabled:true,tone:'#64756d'};
